...
Ticker* WCSSkills::ticker = nullptr;
...
{
	this->ismm = ismm;
	this->ticker = new Ticker(64);
	try {
		Paths::init();
	}
	catch (Paths::WrongDataLocation& e) {
		this->ismm->Format(error, sizeof(error), e.message);
		return false;
	}
...
{
	this->ismm->ConPrintf("Hook_OnClientConnected(%d, \"%s\", %d, \"%s\", \"%s\", %d)\n", slot, pszName, xuid, pszNetworkID, pszAddress, bFakePlayer);
}
...
void WCSSkills::OnLevelInit(char const* pMapName,
	...
	bool background)
{
	this->ismm->ConPrintf("OnLevelInit(%s)\n", pMapName);
}

void WCSSkills::OnLevelShutdown()
{
	this->ismm->ConPrint("OnLevelShutdown()\n");
}
